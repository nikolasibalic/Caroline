...
export function initApp() {
  ...
  if (leftHanded) {
    ...
    c.style.left = "0px";
    rtp.style.left = "0px";
    if (!window.presenter && roundTableAuth) {
  ...
  d[5].addEventListener("click", function (e) {
    document.getElementById("controlspace").children[4].classList.add("image-draw");
    document.getElementById("controlspace").children[4].classList.remove("image-text");
    openSketchPad();
    e.preventDefault();
  });
  ...
  d[12].addEventListener("click", function (e) {
    document.getElementById("controlspace").children[4].classList.remove("image-draw");
    document.getElementById("controlspace").children[4].classList.add("image-text");
    addOrEditText();
    e.preventDefault();
  })
  if (!window.presenter) {
  ...
  if (!roundTable && ((h.indexOf("slide") == 0) || h.length < 10)) {
    if (window.presenter && presentationServer != "") {
      username = "Lecturer";
      addLecturerSempahore();
      lecturerConnection();
    }
    else if (!window.presenter && roundTableAuth) {
      username = "Audience";
      audienceConnection();
...
function openIpynb(im) {
  ...
  fr.addEventListener('loadend', function (e) {
    ...
    const nbv = nbv_constructor(document, {
      "md": markdownit().use(markdownItMathjax()),
      "hljs": hljs
    });
    roundTableData[currentSurface].file = im;
...
function openPDF(im) {
  ...
  */
  getPDFDocument(URL.createObjectURL(im)).promise.then(function (pdfDoc_) {
    pdfDoc = pdfDoc_;
...
function saveScreenshot() {
  ...
  if (annotationAnchor === null) {
    ...
    html2canvas(screenshotTarget, {
      ...
      height: annotationAnchor.scrollHeight / 0.99
    }).then((canvas) => {
      const base64image = canvas.toDataURL("image/png");
      var filename = timestampFilename() + ".png";
      saveAs(base64image, filename);
...
function closeRoundtable() {
  ...
  saveSurface();
  sketchpadOpen = false;
  if (window.presenter && roundTable) {
    socket.emit("send_room",
      {
...
function addCameraStream(deviceId, facing) {
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    navigator.mediaDevices.getUserMedia(request).then(function (stream) {
      ...
      window.presenterVideoStream = stream;
      var video = document.createElement("video");
      video.id = "global.presentervideo";
      ...
      video.addEventListener("click", function () {
        window.presenterVideoStream.getTracks().forEach(function (track) {
...
function resizeSlide() {
  ...
  checkOverflow();
  var c = document.getElementById("global.presentervideo");
  if (c) {
    var w = Math.round(window.innerWidth * 0.1);
    var h = Math.round(w / 640 * 480);
...
function showSlide(doNotPushHistory = false) {
  ...
  if (window.presenter && q.length > 0) {
...
function slide2pdf() {
  ...
  var scale = 1980 / window.innerWidth;
  html2canvas(screenshotTarget, {
    "scale": scale, "logging": false, "useCORS": false,
...
function previousSlide() {
  if ((slideIndex == 0) && window.presenter && (presentationServer != "")) {
    openPointer();
    closeExistingSlide();
    previousIndex = -1;
    sketchpadOpen = false;
...
export function initRoundTable() {
  var h = window.location.hash.substr(1)
...
export function enabaleProceed() {
  document.getElementById("bproceed").disabled = false;
}

export function disableProceed() {
  document.getElementById("bproceed").disabled = true;
}
